...
*/
	{
		...
		if (isset($email) && qa_email_validate($email)) {
            ...
            global $CFG;
            $subs['^url'] = $CFG->wwwroot;
            if (isset($_GET['cmid'])) {
                $cmid = $_GET['cmid'];
                $subs['^url'] = $CFG->wwwroot.'/mod/forumqta/view.php?id='.$cmid;
            }

            $subs['^site_title'] = qa_opt('site_title');
            $headers = 'From: Forum <'.qa_opt('from_email').'>';

            //return mail($email, strtr($subject, $subs), (empty($handle) ? '' : qa_lang_sub('emails/to_handle_prefix', $handle)).strtr($body, $subs),$headers);

            return qa_send_email_leyun(array(
 ...
 * @return bool|mixed
 */
function qa_send_email_leyun($params)
    /*
...
{
    if (qa_to_override(__FUNCTION__)) { $args=func_get_args(); return qa_call_override(__FUNCTION__, $args); }
    //error_log("email called1");
    //error_log(print_r($params, true));

    //require_once QA_INCLUDE_DIR.'qa-class.phpmailer.php'; this phpmailer version is depreched in new PHP
    ...
    try {
        //Parameters recommanded by Informaniak for php 5.6
        ...
        $mailer->Username=SMTP_USENAME;
        $mailer->Password=SMTP_PASSWORD;
        $mailer->SMTPSecure='tls';
        $mailer->Port=SMTP_PORT;

        $mailer->setFrom('no-reply@example.com', $params['fromname']);
