...
*/
	{
        ...
        if (isset($_GET['groupid'])) {
            ...
            $groupid = qa_post_text('qa_groupid');
        }
        ...
        if (isset($_GET['k_1'])) {
            ...
            $categorylimit = " ^posts.categoryid = ".$categoryid;
        }

        if ($groupid <= 0) { //teacher and administrator with all participants OR without seperated group, select all questions in the category
            $selectspec['source'].=" JOIN (SELECT postid FROM ^posts WHERE ".$categorylimit." AND ".
        ...
        else { //student in a group
            $selectspec['source'].=" JOIN (SELECT postid FROM ^posts WHERE ".$categorylimit." AND (^posts.groupid = $groupid OR ^posts.groupid = 0) AND ".
                qa_db_categoryslugs_sql_args($categoryslugs, $selectspec['arguments']).
                (isset($createip) ? "createip=INET_ATON($) AND " : "").
                "type=$ ".$sortsql." LIMIT #,#) y ON ^posts.postid=y.postid";
        }
...
*/
	{
        ...
        if ($groupid <= 0) {
            ...
            $selectspec['source'].=" JOIN (SELECT postid FROM ^posts WHERE (^posts.groupid = $groupid OR ^posts.groupid = 0)  AND ".qa_db_categoryslugs_sql_args($categoryslugs, $selectspec['arguments'])."type=$ AND ".$bysql." AND closedbyid IS NULL ORDER BY ^posts.created DESC LIMIT #,#) y ON ^posts.postid=y.postid";
        }
...
*/
	{
		...
		$selectspec['source'].=" JOIN ^posts AS aposts ON ^posts.postid=aposts.parentid".
			...
			" JOIN (SELECT postid FROM ^posts WHERE ".$categorylimit." AND ".
			qa_db_categoryslugs_sql_args($categoryslugs, $selectspec['arguments']).
			(isset($createip) ? "createip=INET_ATON($) AND " : "").
...
*/
	{
        ...
        if (isset($_GET['k_1'])) {
            $string_k1 = $_GET['k_1'];
            $categoryid = substr(qa_get('k_1'), 1, strlen($string_k1));
            $categorylimit = 'AND categoryid = '.$categoryid;
        }
		...
		$selectspec['source'].=" JOIN ^posts AS parentposts ON".
			...
			" JOIN (SELECT postid FROM ^posts WHERE flagcount>0 AND type IN ('Q', 'A', 'C') ".$categorylimit." ORDER BY ^posts.flagcount DESC, ^posts.created DESC LIMIT #,#) y ON flagposts.postid=y.postid";
...
*/
	{
		//Get the Q2A category tags from the URL
		...
		$selectspec['source'].=" JOIN (SELECT questionid, SUM(score)+2*(LOG(#)*(^posts.hotness-(SELECT MIN(hotness) FROM ^posts WHERE type='Q' AND categoryid=$category_id))/((SELECT MAX(hotness) FROM ^posts WHERE type='Q' AND categoryid=$category_id)-(SELECT MIN(hotness) FROM ^posts WHERE type='Q' AND categoryid=$category_id)))+LOG(questionid)/1000000 AS score, GROUP_CONCAT(CONCAT_WS(':', matchposttype, matchpostid, ROUND(score,3))) AS matchparts FROM (";
        $selectspec['sortdesc']='score';
		...
		if (!empty($titlewords)) {
			...
			"(SELECT qp.postid AS questionid, LOG(#/titlecount) AS score, 'Q' AS matchposttype, qp.postid AS matchpostid FROM ^titlewords qt, ^words qw, ^posts qp WHERE qt.wordid=qw.wordid AND qt.postid = qp.postid AND qp.categoryid = ".$category_id." AND word IN ($) AND tagwordcount<#)";
		...
		if (!empty($contentwords)) {
			...
			$selectspec['source'].=($selectparts++ ? " UNION ALL " : "").
				"(SELECT qc.questionid, (1-1/(1+qc.count))*LOG(#/qw.contentcount)*(CASE qc.type WHEN 'Q' THEN 1.0 WHEN 'A' THEN 0.5 ELSE 0.25 END) AS score, qc.type AS matchposttype, qc.postid AS matchpostid FROM ^contentwords qc, ^words qw, ^posts qp WHERE qc.wordid=qw.wordid AND qc.postid = qp.postid AND qp.categoryid = ".$category_id." AND word IN ($) AND contentcount<#)";
		...
		if (!empty($tagwords)) {
			...
			$selectspec['source'].=($selectparts++ ? " UNION ALL " : "").
				"(SELECT qp.postid AS questionid, 2*LOG(#/tagwordcount) AS score, 'Q' AS matchposttype, qp.postid AS matchpostid FROM ^tagwords qt, ^words qw, ^posts qp WHERE qt.wordid=qw.wordid AND qt.postid = qp.postid AND qp.categoryid = ".$category_id." AND word IN ($) AND tagwordcount<#)";
		...
		if (!empty($handlewords)) {
			if (QA_FINAL_EXTERNAL_USERS) {
				...
				if (count($userids)) {
					$selectspec['source'].=($selectparts++ ? " UNION ALL " : "").
						"(SELECT postid AS questionid, LOG(#/qposts) AS score, 'Q' AS matchposttype, postid AS matchpostid FROM ^posts  JOIN ^userpoints ON ^posts.userid=^userpoints.userid WHERE ^posts.userid IN ($) AND type='Q' AND categoryid=$category_id)";
				...
				$selectspec['source'].=($selectparts++ ? " UNION ALL " : "").
					"(SELECT postid AS questionid, LOG(#/qposts) AS score, 'Q' AS matchposttype, postid AS matchpostid FROM ^posts  JOIN ^users ON ^posts.userid=^users.userid JOIN ^userpoints ON ^userpoints.userid=^users.userid WHERE handle IN ($) AND type='Q' AND categoryid=$category_id)";
		...
		if (strlen($handle)) { // to allow searching for multi-word usernames (only works if search query contains full username and nothing else)
			if (QA_FINAL_EXTERNAL_USERS) {
				...
				if (count($userids)) {
					$selectspec['source'].=($selectparts++ ? " UNION ALL " : "").
						"(SELECT postid AS questionid, LOG(#/qposts) AS score, 'Q' AS matchposttype, postid AS matchpostid FROM ^posts  JOIN ^userpoints ON ^posts.userid=^userpoints.userid WHERE ^posts.userid=$ AND type='Q' AND categoryid=$category_id)";
				...
				$selectspec['source'].=($selectparts++ ? " UNION ALL " : "").
					"(SELECT postid AS questionid, LOG(#/qposts) AS score, 'Q' AS matchposttype, postid AS matchpostid FROM ^posts  JOIN ^users ON ^posts.userid=^users.userid JOIN ^userpoints ON ^userpoints.userid=^users.userid WHERE handle=$ AND type='Q' AND categoryid=$category_id)";
		...
		if ($selectparts==0)
			$selectspec['source'].='(SELECT NULL as questionid, 0 AS score, NULL AS matchposttype, NULL AS matchpostid FROM ^posts WHERE postid IS NULL AND categoryid='.$category_id.')';
...
*/
	{
		...
		$selectspec=array(
			'columns' => array('^categories.id', '^categories.categoryid', '^categories.parentid', 'title' => '^categories.title', 'tags' => '^categories.tags', '^categories.qcount', '^categories.position'),
			'source' => '^categories JOIN ('.implode(' UNION ', $parentselects).') y ON ^categories.parentid<=>parentkey'.($full ? ' LEFT JOIN ^categories AS child ON child.parentid=^categories.categoryid GROUP BY ^categories.id' : '').' ORDER BY ^categories.position',
...
*/
	...
	function qa_db_tag_recent_qs_selectspec($voteuserid, $tag, $start, $full=false, $count=null, $category_tags)
/*
...
*/
	{
		...
		$selectspec['source'].=" JOIN (SELECT qt.postid FROM ^posttags qt, ^posts qp, ^categories qc WHERE qt.postid = qp.postid AND qp.categoryid = qc.categoryid AND qc.tags = '".$category_tags."' AND wordid=(SELECT wordid FROM ^words WHERE word=$ AND word=$ COLLATE utf8_bin LIMIT 1) ORDER BY qt.postcreated DESC LIMIT #,#) y ON ^posts.postid=y.postid";
...
*/
	...
	function qa_db_popular_tags_selectspec($start, $count=null, $categorytags)
/*
...
*/
	{
        ...
        if ($groupid <= 0) { //all participants OR without seperated group
            return array(
                'columns' => array('word', 'tagcount'),
                'source' => '^words JOIN (SELECT qw.wordid FROM ^words qw, ^tagwords qt, ^posts qp, ^categories qc WHERE tagcount>0 AND qc.tags = \''.$categorytags.'\' AND qw.wordid = qt.wordid AND qt.postid = qp.postid AND qc.categoryid = qp.categoryid ORDER BY tagcount DESC LIMIT #,#) y ON ^words.wordid=y.wordid',
            ...
            return array(
                'columns' => array('word', 'tagcount'),
                'source' => '^words JOIN (SELECT qw.wordid FROM ^words qw, ^tagwords qt, ^posts qp, ^categories qc WHERE (qp.groupid = '.$groupid.' OR qp.groupid = 0) AND tagcount>0 AND qc.tags = \''.$categorytags.'\' AND qw.wordid = qt.wordid AND qt.postid = qp.postid AND qc.categoryid = qp.categoryid ORDER BY tagcount DESC LIMIT #,#) y ON ^words.wordid=y.wordid',
                'arguments' => array($start, $count),
                'arraykey' => 'word',
                'arrayvalue' => 'tagcount',
                'sortdesc' => 'tagcount',
    ...
    function qa_db_all_tags_by_category($categorytags)
        /*
        ...
        */
    {
        return array(
            'columns' => array('word', 'tagcount'),
            'source' => '^words JOIN (SELECT qw.wordid FROM ^words qw, ^tagwords qt, ^posts qp, ^categories qc WHERE tagcount>0 AND qc.tags = \''.$categorytags.'\' AND qw.wordid = qt.wordid AND qt.postid = qp.postid AND qc.categoryid = qp.categoryid ORDER BY tagcount DESC) y ON ^words.wordid=y.wordid',
            'arraykey' => 'word',
            'arrayvalue' => 'tagcount',
            'sortdesc' => 'tagcount',
        );
...
*/
    ...
    function qa_db_tag_count($word)
    /*
        Return the number of occurrence of a tag
    */
    {
        if (isset($_GET['k_1'])) {
            $string_k1 = $_GET['k_1'];
        ...
        return array(
            'columns' => array('tagcount' => ('COUNT(*)')),
            'source' => '^posts p, ^words w, ^tagwords t WHERE t.wordid = w.wordid AND t.postid = p.postid AND w.word = # AND p.categoryid = #',
            'arguments' => array($word, $categoryid)
        );
    }
