			...
			switch ($event) {
				case 'q_post':
                    ...
                    if (isset($_GET['cmid'])) {
                        ...
                        $users = get_enrolled_users($contextcourse,'mod/forum:viewdiscussion');
                        $authorid = $USER->id;
                        foreach ($users as $user) {
                            if ($authorid != $user->id) {
                                qa_send_notification(null, $user->email, null, qa_lang('emails/q_posted_subject'), qa_lang('emails/q_posted_body'), array(
                                    '^q_handle' => $sendhandle,
                                    '^q_course_title' => $course->fullname,
                                    '^q_title' => $params['title'], // don't censor title or content here since we want the admin to see bad words
                                    '^q_content' => $params['text'],
                                    '^url' => qa_q_path($params['postid'], $params['title'], true),
                                ));
