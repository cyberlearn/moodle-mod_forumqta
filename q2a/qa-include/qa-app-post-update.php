...
*/
	{
        ...
        $params = array(
            'context' => $context,
            'objectid' => $oldquestion['postid'],
        );
        $event = \mod_forumqta\event\question_updated::create($params);
        $event->trigger();
	}
...
*/
	{
		...
		if (isset($oldselchildid) && isset($answers[$oldselchildid])) {
            ...
            $event = \mod_forumqta\event\answer_unselected::create($params);
            $event->trigger();
		}
		...
		if (isset($selchildid)) {
            ...
            $event = \mod_forumqta\event\answer_selected::create($params);
            $event->trigger();
		}
	}
...
*/
	{
		...
		if (isset($oldquestion['closedbyid'])) {
            ...
            $event = \mod_forumqta\event\question_reopened::create($params);
            $event->trigger();
		}
        else {
            $event = \mod_forumqta\event\question_closed::create($params);
            $event->trigger();
        }
	}
...
*/
	{
        ...
        if ($hidden) {
            if(is_null($userid)) {
                $event = \mod_forumqta\event\post_flag_hidden::create($params);
            }
            else {
                $event = \mod_forumqta\event\question_hidden::create($params);
            }
        }
...
*/
	{
		...
		if ($status==QA_POST_STATUS_QUEUED) {
            ...
            $event = \mod_forumqta\event\question_reshowed::create($params);
            $event->trigger();
...
*/
	{
        ...
        $params = array(
            'context' => $context,
            'objectid' => $oldanswer['postid'],
        );
        $event = \mod_forumqta\event\answer_updated::create($params);
        $event->trigger();
	}
...
*/
	{
        ...
        if ($status==QA_POST_STATUS_HIDDEN) {
            $event = \mod_forumqta\event\answer_hidden::create($params);
        }
        elseif ($status==QA_POST_STATUS_NORMAL) {
            $event = \mod_forumqta\event\answer_reshowed::create($params);
        }
        $event->trigger();
...
*/
	{
        ...
        $params = array(
            'context' => $context,
            'objectid' => $oldcomment['postid'],
        );
        $event = \mod_forumqta\event\comment_updated::create($params);
        $event->trigger();
	}
...
*/
	{
        ...
        if ($status==QA_POST_STATUS_HIDDEN) {
            $event = \mod_forumqta\event\comment_hidden::create($params);
        }
        elseif ($status==QA_POST_STATUS_NORMAL) {
            $event = \mod_forumqta\event\comment_reshowed::create($params);
        }
        $event->trigger();
