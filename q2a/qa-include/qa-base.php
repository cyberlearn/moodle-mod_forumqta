    ...
    */
{
	...
	$languagecode = $course->lang;

	if (strcmp($languagecode, '') == 0) {
		$moodleconfig = $DB->get_record('config', array('name'=>'lang'));
		$languagecode = $moodleconfig->value;
	}
...
{
    if (isset($_GET['k_1'])) {
        $category_tags = qa_get('k_1');
    }
    else {
        $category_tags = qa_post_text('qa_k_1');
    }
		...
		case QA_URL_FORMAT_PARAMS:
			if (!empty($request))
				foreach ($requestparts as $partindex => $requestpart)
					$paramsextra.=(strlen($paramsextra) ? '&' : '?').'k_1='.$category_tags.'&groupid='.$groupid.'&cmid='.$cmid.'&qa'.($partindex ? ('_'.$partindex) : '').'='.$requestpart;
			break;
	}
	...
	//add a filter for the specified category

	if (strcmp($request, 'questions') == 0 || strcmp($request, 'unanswered') == 0)
		$paramsextra.='&qa_1='.$category_tags;
