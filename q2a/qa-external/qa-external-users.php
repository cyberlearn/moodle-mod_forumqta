...
*/
	{
            ...
            elseif (has_capability('moodle/grade:viewall', $context)) {
                ...
                if ($groupid == -1) { //without seperated group and visible group
                    $level = QA_USER_LEVEL_EXPERT;
                }
                else {
                    //qa_get() and $_GET do not work for getting 'qa' value
                    $qastring = qa_post_text('qa_qa');
                    if (!isset($qastring)) {
                        $querystring = $_SERVER['QUERY_STRING'];
                        $pos1 = strpos($querystring, 'qa=');
                        $pos2 = strpos($querystring, '&', $pos1);
                        $qastring = substr($querystring, $pos1+3, $pos2-$pos1-3);
                    }
                    $postid = intval($qastring, 10);
                    if ($postid != 0) { // This is a question
                        $forumpost = $DB->get_record('forumqta_posts', array('postid'=>$postid));
                        $result_groupid = $forumpost->groupid;
                        if ($result_groupid != 0) { //not a teacher & admin "global" post
                            if (groups_is_member($result_groupid, $userid_moodle)) {
                                $level = QA_USER_LEVEL_EXPERT;
                            }
                            else {
                                $level = QA_USER_LEVEL_BASIC;
                        ...
                        if ($groupid == 0) { //All the members
                            $level = QA_USER_LEVEL_BASIC;
                        }
                        else {
                            if (groups_is_member($groupid, $userid_moodle)) {
                                $level = QA_USER_LEVEL_EXPERT;
                            }
                            else {
                                $level = QA_USER_LEVEL_BASIC;
...
*/
	...
	//	Until you edit this function, always return null
        ...
        if (count($publicusernames)) {
            ...
            $table = 'user';
            $select = 'username IN ('.implode(',', $escapedusernames).')'; //is put into the where clause
...
*/
	...
	//	Until you edit this function, always return null
        ...
        if (count($userids)) {
            ...
            $table = 'user';
            $select = 'id IN ('.implode(',', $escapeduserids).')'; //is put into the where clause
            $results = $DB->get_records_select($table,$select);

            foreach ($results as $result) {
                $useridtopublic[$result->id]=$result->firstname.' '.$result->lastname;
            }
        }
...
*/
	...
	//	By default, show the public username linked to the Q2A profile page for each user
		...
		foreach ($userids as $userid) {
			if($userid!=0){
                $urluserprofile = new moodle_url("/user/profile.php?",array('id' => $userid));
				$publicusername=$useridtopublic[$userid];
				$usershtml[$userid]=htmlspecialchars($publicusername);
				if ($should_include_link)
					$usershtml[$userid]='<span class="qa-user-link"><a target="_parent" href="'.$urluserprofile.'">'.$publicusername.'</a></span>'; // Supression du lien pour renvoyer aux utilisateurs
